...
class DungeonMap {
    ...
    constructor(
        ...
        this.totalFights = ko.observable(this.board().flat().flat().filter((t: DungeonTile) => t.type() == DungeonTileType.enemy).length);
        this.totalChests = ko.observable(this.board().flat().flat().filter((t: DungeonTile) => t.type() == DungeonTileType.chest).length);
    }
    ...
    public findShortestPath(start: Point, goal: Point, avoidTiles: DungeonTileType[] = []) {
        const pathing = [start];
        const fromPos: { [key: string]: Point | null } = {};
        fromPos[`${start.x},${start.y}`] = null;
        while (pathing.length > 0) {
            const current = pathing.shift()!; // current is defined, because pathing.length > 0 (while condition)
     ...
     */
    public shuffle(a: DungeonTile[]): void {
        let j, x, i;
        for (i = a.length; i; i--) {
            j = Rand.floor(i);
            x = a[i - 1];
