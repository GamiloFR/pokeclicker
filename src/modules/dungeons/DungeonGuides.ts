...
class DungeonGuides {
    ...
    public static available = ko.pureComputed(() => DungeonGuides.list.filter(f => f.isUnlocked()));
    public static selected = ko.observable(0).extend({ numeric: 0 });
    public static hired = ko.observable<DungeonGuide | null>(null);
    public static clears = ko.observable(1).extend({ numeric: 0 });
    ...
    public static canAfford(): boolean {
        const costs: Partial<Record<Currency, Amount>> = {
...
DungeonGuides.add(new DungeonGuide('Timmy', 'Can smell when there is a treasure chest on a tile near them!',
    ...
    () => {
        ...
        const treasureTiles = (<DungeonTile[][][]>DungeonRunner.map.board())[pos.floor].flat().filter(t => t.type() == DungeonTileType.chest);
...
DungeonGuides.add(new DungeonGuide('Shelly', 'Prefers to explore the unknown!',
    ...
    () => {
        ...
        const unexploredTiles = (<DungeonTile[][][]>DungeonRunner.map.board())[pos.floor].flat().filter(t => !t.isVisited);
...
DungeonGuides.add(new DungeonGuide('Angeline', 'Can find treasure anywhere, loves to explore new areas!',
    [[15, Currency.money], [10, Currency.dungeonToken]], [new Amount(150, Currency.diamond)],
    1000,
    () => {
        // Get current position
        const pos = DungeonRunner.map.playerPosition();
...
DungeonGuides.add(new DungeonGuide('Georgia', 'Knows the path to the boss, avoids random encounters when possible.',
    [[20, Currency.money], [20, Currency.dungeonToken]], [new Amount(300, Currency.diamond)],
    900,
    () => {
        // Get current position
        const pos = DungeonRunner.map.playerPosition();
        ...
        const bossPosition = DungeonRunner.map.board()[pos.floor].flat().find((t: DungeonTile) => t.type() == DungeonTileType.boss)?.position;
        const ladderPosition = DungeonRunner.map.board()[pos.floor].flat().find((t: DungeonTile) => t.type() == DungeonTileType.ladder)?.position;
...
DungeonGuides.add(new DungeonGuide('Drake', 'Knows the shortest path to the boss!',
    [[20, Currency.money], [20, Currency.dungeonToken]], [new Amount(450, Currency.diamond)],
    800,
    () => {
        // Get current position
        const pos = DungeonRunner.map.playerPosition();
