...
class DungeonGuides {
    ...
    public static available: Computed<DungeonGuide[]> = ko.pureComputed(() => DungeonGuides.list.filter((f) => f.isUnlocked()));
    public static selected: Observable<number> = ko.observable(0).extend({ numeric: 0 });
    public static hired: Observable<DungeonGuide> = ko.observable(null);
    public static clears: Observable<number> = ko.observable(1).extend({ numeric: 0 });
    ...
    public static getRandomWeightedNearbyTile(nearbyTiles: DungeonTile[]): DungeonTile {
        const weightedTiles = nearbyTiles.map((t) => {
            ...
            weight += [DungeonTileType.enemy].includes(t.type()) ? 1 : 0;
            weight += [DungeonTileType.chest].includes(t.type()) ? 2 : 0;
            weight += [DungeonTileType.boss, DungeonTileType.ladder].includes(t.type()) ? 4 : 0;
            return weight;
        });
...
class DungeonGuide {
    ...
    public hired: Observable<boolean> = ko.observable(false).extend({ boolean: null });
            ...
            try {
                ...
                switch (DungeonRunner.map.currentTile().type()) {
                    ...
                    case DungeonTileType.ladder:
                        DungeonRunner.handleInteraction(DungeonInteractionSource.DungeonGuide);
                        break;
                }
            } catch (e) {
    ...
    calcCost(clears: number, price: number, region: Region, includeDungeonCost = false): Amount[] {
        const costs = [];
        ...
        if (includeDungeonCost) {
            let dtCost = costs.find((c) => c.currency === Currency.dungeonToken);
            if (!dtCost) {
                dtCost = new Amount(0, Currency.dungeonToken);
                costs.push(dtCost);
            }
...
DungeonGuides.add(
    new DungeonGuide(
        'Timmy',
        'Can smell when there is a treasure chest on a tile near them!',
        [
            [4, Currency.money],
            [1, Currency.dungeonToken],
        ],
        [],
        2000,
        () => {
            ...
            const treasureTiles = DungeonRunner.map
                .board()
                [pos.floor].flat()
                .filter((t) => t.type() == DungeonTileType.chest);
...
DungeonGuides.add(
    new DungeonGuide(
        'Shelly',
        'Prefers to explore the unknown!',
        [
            [4, Currency.money],
            [4, Currency.dungeonToken],
        ],
        [new Amount(5, Currency.questPoint)],
        1500,
        () => {
            // Get current position
            const pos = DungeonRunner.map.playerPosition();
            ...
            const unexploredTiles = DungeonRunner.map
                .board()
                [pos.floor].flat()
                .filter((t) => !t.isVisited);
...
DungeonGuides.add(
    new DungeonGuide(
        'Angeline',
        'Can find treasure anywhere, loves to explore new areas!',
        [
            [15, Currency.money],
            [10, Currency.dungeonToken],
        ],
        [new Amount(150, Currency.diamond)],
        1000,
        () => {
            // Get current position
            const pos = DungeonRunner.map.playerPosition();
...
DungeonGuides.add(
    new DungeonGuide(
        'Georgia',
        'Knows the path to the boss, avoids random encounters when possible.',
        [
            [20, Currency.money],
            [20, Currency.dungeonToken],
        ],
        [new Amount(300, Currency.diamond)],
        900,
        () => {
            // Get current position
            const pos = DungeonRunner.map.playerPosition();
            ...
            const bossPosition = DungeonRunner.map
                .board()
                [pos.floor].flat()
                .find((t) => t.type() == DungeonTileType.boss)?.position;
            const ladderPosition = DungeonRunner.map
                .board()
                [pos.floor].flat()
                .find((t) => t.type() == DungeonTileType.ladder)?.position;

            // Shortest path to the boss avoiding enemies
            let path = bossPosition || ladderPosition ? DungeonRunner.map.findShortestPath(pos, bossPosition || ladderPosition, [DungeonTileType.enemy]) : [];
...
DungeonGuides.add(
    new DungeonGuide(
        'Drake',
        'Knows the shortest path to the boss!',
        [
            [20, Currency.money],
            [20, Currency.dungeonToken],
        ],
        [new Amount(450, Currency.diamond)],
        800,
        () => {
            // Get current position
            const pos = DungeonRunner.map.playerPosition();
