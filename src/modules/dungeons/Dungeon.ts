...
class Dungeon {
        ...
        this.enemyList.forEach((enemy) => {
            ...
            if (typeof enemy === 'string' || enemy.hasOwnProperty('pokemon')) {
                ...
                if (enemy.hasOwnProperty('pokemon')) {
                    ...
                    const requirement = (enemy as DetailedPokemon).options.requirement;
    ...
    public getMimicData(pokemonName: PokemonNameType): MimicData {
        let res;
    ...
    get normalEncounterList(): EncounterInfo[] {
        ...
        this.enemyList.forEach((enemy) => {
            ...
            if (typeof enemy === 'string' || enemy.hasOwnProperty('pokemon')) {
                ...
                encounterInfo.push(this.getEncounterInfo(pokemonName, undefined, hideEncounter));
            // Handling Trainers (only those with shadow Pokemon)
                ...
                if (shadowPokemon.length) {
                    const shadowEncounters = shadowPokemon.map(p => this.getEncounterInfo(p.name, undefined, hideEncounter, true));
    ...
    get bossEncounterList(): EncounterInfo[] {
        ...
        this.bossList.forEach((boss) => {
            ...
            // Handling Pokemon
            if (boss instanceof DungeonBossPokemon) {
                const encounter = this.getEncounterInfo(boss.name, undefined, hideEncounter);
                encounter.lock = lock;
    ...
    private getEncounterInfo(pokemonName: PokemonNameType, mimicData?: MimicData, hideEncounter = false, shadow = false): EncounterInfo {
