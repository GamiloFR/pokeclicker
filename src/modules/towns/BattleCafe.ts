...
export class BattleCafeController {
    ...
    // Give additional spins for each sweet type completed, shiny, and resistant
        ...
        const sweetStatus = GameHelper.enumStrings(GameConstants.AlcremieSweet).map((s) => ({
    ...
    public static canBuySweet(sweet: GameConstants.AlcremieSweet): Computed<boolean> {
    ...
    public static getCaughtStatus(sweet: GameConstants.AlcremieSweet): Computed<CaughtStatus> {
        return ko.pureComputed(() => {
            return Math.min(...Object.values(BattleCafeController.evolutions[sweet]).map((pokemon: PokemonItem) => pokemon.getCaughtStatus()));
        });
    }

    public static getPokerusStatus(sweet: GameConstants.AlcremieSweet): Computed<GameConstants.Pokerus> {
        return ko.pureComputed(() => {
    ...
    public static calcMaxSpins(sweet: GameConstants.AlcremieSweet): number {
        const maxSpins = BattleCafeController.getPrice(sweet).map((cost) => Math.floor(App.game.farming.berryList[cost.berry]() / cost.amount));
        return Math.min(...maxSpins);
    }
}

export class BattleCafeSaveObject implements Saveable {
    saveKey = 'BattleCafe';
...
class BattleCafe extends TownContent {
    ...
    public areaStatus(): areaStatus[] {
        ...
        const alcremieList = Object.values(BattleCafeController.evolutions)
            .flatMap((sweet) => Object.values(sweet))
            .map((pi) => pi.type);
