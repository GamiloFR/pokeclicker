...
class Quests implements Saveable {
    ...
    public questList = ko.observableArray<Quest>();
    public questLines = ko.observableArray<QuestLine>();
    public level = ko.pureComputed((): number => {
        return this.xpToLevel(this.xp());
    });
    public questSlots = ko.pureComputed((): number => {
        // Minimum of 1, Maximum of 4
        return Math.min(4, Math.max(1, Math.floor((this.level() + 5) / 5)));
    });

    // Get current quests by status
    public completedQuests = ko.pureComputed(() => {
        return this.sortedQuestList().filter(quest => quest.claimed());
    });
    public currentQuests = ko.pureComputed(() => {
        return this.questList().filter(quest => quest.inProgress());
    });
    public incompleteQuests =  ko.pureComputed(() => {
    ...
    static questCompareBy(quest1: Quest, quest2: Quest): number {
    ...
    static getQuestSortStatus(quest: Quest): number {
     ...
     */
    getQuestLine(name: QuestLineNameType): QuestLine {
        const questLine = this.questLines().find(ql => ql.name.toLowerCase() == name.toLowerCase());
        if (!questLine) {
            throw new Error(`Cannot find questLine ${name}`);
        }
        return questLine;
    }
    ...
    public getRefreshCost(): Amount {
        ...
        return new Amount(Math.max(0, Math.min(1e6, cost)), Currency.money);
    }
    ...
    loadQuestList(questList: any[]) {
        // Sanity Check
        this.questList.removeAll();
        questList.forEach(questData => {
            try {
                if (questData.hasOwnProperty('name')) {
                    const quest = QuestBuilder.createQuest(questData.name, questData.data);
                ...
                console.error(`Quest "${questData.name}" failed to load`, questData, e);
                this.questList.push(new CapturePokemonsQuest(10, 1));
            }
        });
    }

    loadQuestLines(questLines: any[]) {
                ...
                if (ql) {
                    ...
                    if (questLine.state == QuestLineState.started || questLine.state == QuestLineState.suspended) {
                        if (ql.quests()[questLine.quest] instanceof MultipleQuestsQuest) {
                            ...
                            ql.curQuestObject().quests.forEach((q: Quest, i: number) => {
