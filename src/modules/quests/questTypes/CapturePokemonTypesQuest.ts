...
class CapturePokemonTypesQuest extends Quest implements QuestInterface {
    ...
    constructor(capturesNeeded: number, reward: number, public type: PokemonType) {
        ...
        this.focus = ko.pureComputed(() =>
            pokemonMap.filter(p => p.type.includes(this.type)).map(p => App.game.statistics.pokemonCaptured[p.id]()).reduce((a, b) => a + b, 0),
    ...
    private static calcReward(amount: number, type: PokemonType): number {
        const reward = amount * CAPTURE_POKEMONS_BASE_REWARD * this.weights[type].weight * 2;
...
export default CapturePokemonTypesQuest;
