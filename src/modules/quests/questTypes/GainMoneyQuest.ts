...
class GainMoneyQuest extends Quest implements QuestInterface {
    ...
    public static generateData(): any[] {
        ...
        const gymAmount = Object.values(GymList).reduce((max, gym) => {
            if (App.game.statistics.gymsDefeated[getGymIndex(gym.town)]()) {
                ...
                return Math.max(max, (gym.moneyReward) * 1.3 ** (highestRegion - getGymRegion(gym.town)));
            }
            return max;
        }, 0) || GymList[KantoGyms[0]].moneyReward;
    ...
    private static calcReward(amount: number, baseAmount: number): number {
        const reward = Math.ceil(amount / baseAmount * GAIN_MONEY_BASE_REWARD);
