...
class UseOakItemQuest extends Quest implements QuestInterface {
    ...
    private static calcReward(amount: number) {
        const reward = amount * USE_OAK_ITEM_BASE_REWARD;
        return super.randomizeReward(reward);
    }

    get defaultDescription(): string {
        const desc = [];
        desc.push(`Equip the ${humanifyString(OakItemType[this.item])} and`);
...
export default UseOakItemQuest;
