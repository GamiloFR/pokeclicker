...
class GameController {
    ...
    static addKeyListeners() {
        ...
        $oakItemsModal.on("hide.bs.modal", (_) =>
            $oakItemsModal.data("disable-toggle", true)
        );
        $oakItemsModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $oakItemsModal.data("disable-toggle", false)
        );
        // Pokeball Selector
        const pokeballs = App.game.pokeballs;
        const $pokeballSelector = $("#pokeballSelectorModal");
        $pokeballSelector.on("hide.bs.modal", (_) =>
            $pokeballSelector.data("disable-toggle", true)
        );
        $pokeballSelector.on("hidden.bs.modal shown.bs.modal", (_) =>
            $pokeballSelector.data("disable-toggle", false)
        );
        // Underground
        const underground = App.game.underground;
        const $undergroundModal = $("#mineModal");
        $undergroundModal.on("hide.bs.modal", (_) =>
            $undergroundModal.data("disable-toggle", true)
        );
        $undergroundModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $undergroundModal.data("disable-toggle", false)
        );
        // Quests
        const quests = App.game.quests;
        const $questModal = $("#QuestModal");
        $questModal.on("hide.bs.modal", (_) =>
            $questModal.data("disable-toggle", true)
        );
        $questModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $questModal.data("disable-toggle", false)
        );
        // Farm
        const farms = App.game.farming;
        const $farmsModal = $("#farmModal");
        $farmsModal.on("hide.bs.modal", (_) =>
            $farmsModal.data("disable-toggle", true)
        );
        $farmsModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $farmsModal.data("disable-toggle", false)
        );
        // Hatchery
        const hatchery = App.game.breeding;
        const $hatcheryModal = $("#breedingModal");
        $hatcheryModal.on("hide.bs.modal", (_) =>
            $hatcheryModal.data("disable-toggle", true)
        );
        $hatcheryModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $hatcheryModal.data("disable-toggle", false)
        );
        // Achievements Tracker
        const achievements = App.game.achievementTracker;
        const $achievementsModal = $("#achievementsModal");
        $achievementsModal.on("hide.bs.modal", (_) =>
            $achievementsModal.data("disable-toggle", true)
        );
        $achievementsModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $achievementsModal.data("disable-toggle", false)
        );
        // Shop
        const $shopModal = $("#shopModal");
        $shopModal.on("hide.bs.modal", (_) =>
            $shopModal.data("disable-toggle", true)
        );
        $shopModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $shopModal.data("disable-toggle", false)
        );
        // Castform App (Weather)
        const $weatherModal = $("#weatherAppModal");
        $weatherModal.on("hide.bs.modal", (_) =>
            $weatherModal.data("disable-toggle", true)
        );
        $weatherModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $weatherModal.data("disable-toggle", false)
        );
        // Purify Chamber
        const $purifyChamberModal = $("#purifyChamberModal");
        $purifyChamberModal.on("hide.bs.modal", (_) =>
            $purifyChamberModal.data("disable-toggle", true)
        );
        $purifyChamberModal.on("hidden.bs.modal shown.bs.modal", (_) =>
            $purifyChamberModal.data("disable-toggle", false)
        );
        // Ship
        const $shipModal = $("#ShipModal");
        // Modal Collapse
        $(GameConstants.ModalCollapseList)
            .map(function () {
                const id = `#${this}`;
                const method = Settings.getSetting(`modalCollapse.${this}`)
                    .value
                    ? "show"
                    : "hide";
                $(id).collapse(method);
        ...
        $(document).on("keydown", (e) => {
            ...
            if (isNumberKey) {
                if (
                    GameController.keyHeld[
                        Settings.getSetting("hotkey.pokeballSelection").value
                    ]?.()
                ) {
                    // Open pokeball selector modal using P + (1-4) for each condition
                    if (
                        !$pokeballSelector.data("bs.modal")?._isShown &&
                        !$pokeballSelector.data("disable-toggle")
                    ) {
                        $(".modal").modal("hide");
                        $("#pokeballSelectorBody .clickable.pokeball-selected")
                            .eq(numberKey)
                            ?.trigger("click");
            ...
            if ($farmsModal.data("bs.modal")?._isShown) {
                switch (key) {
                    case Settings.getSetting("hotkey.farm.toggleShovel").value:
                        [FarmingTool.Shovel, FarmingTool.MulchShovel].includes(
                            FarmController.selectedFarmTool()
                        )
                            ? FarmController.selectedFarmTool(
                                  FarmController.berryListVisible()
                                      ? FarmingTool.Berry
                                      : FarmingTool.Mulch
                              )
                            : FarmController.selectedFarmTool(
                                  FarmController.berryListVisible()
                                      ? FarmingTool.Shovel
                                      : FarmingTool.MulchShovel
                              );
                        return e.preventDefault();
                    case Settings.getSetting("hotkey.farm.togglePlotSafeLock")
                        .value:
            ...
            if ($undergroundModal.data("bs.modal")?._isShown) {
                switch (key) {
                    case Settings.getSetting("hotkey.underground.hammer").value:
                        App.game.underground.tools.selectedToolType =
                            UndergroundToolType.Hammer;
                        return e.preventDefault();
                    case Settings.getSetting("hotkey.underground.chisel").value:
                        App.game.underground.tools.selectedToolType =
                            UndergroundToolType.Chisel;
                        return e.preventDefault();
                    ...
                    case Settings.getSetting("hotkey.underground.survey").value:
                        ...
                        return e.preventDefault();
                    case Settings.getSetting("hotkey.underground.discharge")
                        .value:
            ...
            if ($pokeballSelector.data("bs.modal")?._isShown) {
                if (isNumberKey) {
                    ...
                    if (
                        GameController.keyHeld[
                            Settings.getSetting("hotkey.pokeballSelection")
                                .value
                        ]?.()
                    ) {
                        $("#pokeballSelectorBody .clickable.pokeball-selected")
            ...
            if ($shipModal.data("bs.modal")?._isShown) {
                if (isNumberKey) {
                    if (numberKey <= player.highestRegion()) {
                        const regionButton = $(".ship-modal-region-button").eq(
                            numberKey
                        );
            ...
            if ($shopModal.data("bs.modal")?._isShown) {
                if (isNumberKey) {
                    if (
                        numberKey <= ShopHandler.shopObservable().items.length
                    ) {
            ...
            if (visibleModals === 0) {
                ...
                if (
                    ...
                    !GameController.keyHeld.Control?.()
                ) {
                    const cycle = Routes.getRoutesByRegion(player.region)
                    ...
                    if (cycle.length > 1) {
                        ...
                        switch (key) {
                            ...
                            case "-":
                                MapHelper.moveToRoute(
                                    cycle[
                                        (idx + cycle.length - 1) % cycle.length
                                    ],
                                    player.region
                                );
                                return e.preventDefault();
                ...
                if (App.game.gameState === GameConstants.GameState.dungeon) {
                    switch (key) {
                        ...
                        case Settings.getSetting("hotkey.dungeon.right").value:
                            ...
                            return e.preventDefault();
                        case Settings.getSetting("hotkey.dungeon.interact")
                            .value:
                            DungeonRunner.handleInteraction(
                                GameConstants.DungeonInteractionSource.Keybind
                            );
                ...
                if (App.game.gameState === GameConstants.GameState.town) {
                    if (
                        key === Settings.getSetting("hotkey.town.start").value
                    ) {
                        ...
                        const filteredContent = player.town.content.filter(
                            (c) => c.isVisible()
                        );
                        const filteredNPCs = player.town.npcs?.filter((n) =>
                            n.isVisible()
                        );
                        if (numberKey < filteredContent.length) {
                            ...
                            filteredNPCs &&
                            numberKey <
                                filteredContent.length + filteredNPCs.length
                        ) {
                            NPCController.openDialog(
                                filteredNPCs[numberKey - filteredContent.length]
                            );
                        }
                        return e.preventDefault();
                    } else if (
                        ...
                        const cycle = Object.values(TownList).filter(
                            (t) =>
                                t instanceof DungeonTown &&
                                t.region == player.region &&
                                t.isUnlocked()
                        );
                        const idx = cycle.findIndex(
                            (d) => d.name == player.town.name
                        );
                        switch (key) {
                            case "=":
                            case "+":
                                MapHelper.moveToTown(
                                    cycle[(idx + 1) % cycle.length].name
                                );
                                return e.preventDefault();
                            case "-":
                                MapHelper.moveToTown(
            ...
            switch (key) {
                ...
                case Settings.getSetting("hotkey.achievementsTracker").value:
                    ...
                    if (
                        achievements.canAccess() &&
                        !$achievementsModal.data("disable-toggle")
                    ) {
                        $(".modal").modal("hide");
                ...
                case Settings.getSetting("hotkey.oakItems").value:
                    ...
                    if (
                        oakItems.canAccess() &&
                        !$oakItemsModal.data("disable-toggle")
                    ) {
                        $(".modal").modal("hide");
                ...
                case Settings.getSetting("hotkey.mute").value:
                    if (GameController.keyHeld.Shift()) {
                        (
                            Settings.getSetting("sound.muted") as BooleanSetting
                        ).toggle();
                        return e.preventDefault();
                    }
                    break;
                ...
                case Settings.getSetting("hotkey.castformApp").value:
                    if (
                        WeatherApp.shortcutVisible() &&
                        !$weatherModal.data("disable-toggle")
                    ) {
                        $(".modal").modal("hide");
                ...
                case Settings.getSetting("hotkey.purifyChamber").value:
                    if (
                        ...
                        !$purifyChamberModal.data("disable-toggle")
                    ) {
                        $(".modal").modal("hide");
