...
class PokemonLocations {
    ...
    */
    ...
    private static readonly pokemonNames: string[] = pokemonList.map(
        (p) => p.name
    );

    private static getCache<T>(cacheName: string) {
        let cache: { [name: string]: T } =
            this.pokemonLocationsCache[cacheName];
        if (!cache) {
    ...
    private static initRegionalCacheLine<T>(
        cache,
    ...
    private static initCacheLine<T>(
        cacheLine,
        defaultValue: { new (): T }
    ): { [name: string]: T } {
        this.pokemonNames.forEach(
            (name) => (cacheLine[name] = new defaultValue())
        );
        return cacheLine;
    }

    public static getPokemonRegionRoutes(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): { [name: string]: Array<object> } {
        const cache = this.getRegionalCache<{ [name: string]: Array<object> }>(
            this.getPokemonRegionRoutes.name
        );
        if (cache[maxRegion]) {
        ...
        Routes.regionRoutes.forEach((routeData) => {
            ...
            Object.entries(routeData.pokemon).forEach(
                ([encounterType, pokemon]) => {
                    new Set(Object.values(pokemon).flat()).forEach(
                        (name: any) => {
                            ...
                            cacheLine[name][region].push({
                                route: routeData.number,
                            });
                        }
                    );
                }
    ...
    public static getPokemonDungeons(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<object> {
        const cache = this.getRegionalCache<object[]>(
            this.getPokemonDungeons.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        const cacheLine = this.initRegionalCacheLine(
            cache,
            maxRegion,
            Array<object>
        );
    ...
    public static getPokemonBossDungeons(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<object> {
        const cache = this.getRegionalCache<object[]>(
            this.getPokemonBossDungeons.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
    ...
    public static getPokemonChestDungeons(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<object> {
        const cache = this.getRegionalCache<object[]>(
            this.getPokemonChestDungeons.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        Object.entries(DungeonList).forEach(([dungeonName, dungeon]) => {
            ...
            // Dungeon Chest
            Object.values(dungeon.lootTable)
                .flat()
                .forEach((i) => {
    ...
    public static getShadowPokemonDungeons(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        const cache = this.getRegionalCache<string[]>(
            this.getShadowPokemonDungeons.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
    ...
    public static getPokemonEggs(
        ...
        Object.entries(App.game.breeding.hatchList).forEach(
            ([eggItemType, eggArr]) => {
                eggArr.forEach((pokemonArr, region) => {
                    // If we only want to check up to a maximum region
                    if (
                        maxRegion != GameConstants.Region.none &&
                        region > maxRegion
                    ) {
                        return false;
                    }
                    pokemonArr.forEach((name) => {
                        cacheLine[name].push(
                            GameConstants.EggItemType[eggItemType]
                        );
                    });
                });
            }
    ...
    public static getPokemonShops(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        const cache = this.getRegionalCache<string[]>(
            this.getPokemonShops.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        Object.entries(TownList).forEach(([townName, town]) => {
            ...
            const townShops = town.content.filter(
                (c) => c instanceof Shop && c.items
            );
            if (townShops.length) {
                // If we only want to check up to a maximum region
    ...
    public static getPokemonRoamingRegions(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<object> {
        const cache = this.getRegionalCache<object[]>(
            this.getPokemonRoamingRegions.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        Object.entries(RoamingPokemonList.list).forEach(
            ([region, regionArr]) => {
                if (
                    maxRegion != GameConstants.Region.none &&
                    +region > maxRegion
    ...
    public static getPokemonParents(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        const cache = this.getRegionalCache<string[]>(
            this.getPokemonParents.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        Object.entries(pokemonBabyPrevolutionMap).forEach(([parent, baby]) => {
            if (
                maxRegion != GameConstants.Region.none &&
                (pokemonMap[parent].nativeRegion > maxRegion ||
                    pokemonMap[pokemonName].nativeRegion > maxRegion)
            ) {
                return false;
            }
    ...
    public static getPokemonSafariChance(
        pokemonName: PokemonNameType
    ): Record<GameConstants.Region, Record<number, number>> {
        const cache = this.getCache<
            Record<GameConstants.Region, Record<number, number>>
        >(this.getPokemonSafariChance.name);
        ...
        Object.entries(SafariPokemonList.list).forEach(([region]) => {
            ...
            const safariWeight = zoneList.reduce(
                (sum, p) => (sum += p.weight),
                0
            );
            zoneList.forEach((safariPokemon) => {
                cacheLine[safariPokemon.name][+region] =
                    cacheLine[safariPokemon.name][+region] || {};
                cacheLine[safariPokemon.name][+region][0] = +(
                    (SafariPokemon.calcPokemonWeight(safariPokemon) /
                        safariWeight) *
                    100
                ).toFixed(2);
            });
        });
        return cacheLine[pokemonName] as Record<
            GameConstants.Region,
            Record<number, number>
        >;
    }

    public static getPokemonPrevolution(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<EvoData> {
        const cache = this.getRegionalCache<EvoData[]>(
            this.getPokemonPrevolution.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        const prevolutionPokemon = pokemonList.filter(
            (p: PokemonListData) => p.evolutions
        );
        prevolutionPokemon.forEach((p: PokemonListData) =>
            p.evolutions.forEach((e) => {
                ...
                if (
                    ...
                    (p.nativeRegion > maxRegion ||
                        pokemonMap[e.evolvedPokemon].nativeRegion > maxRegion)
                ) {
                    return false;
                }
    ...
    public static getPokemonLevelPrevolution(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): EvoData {
        if (
        ...
        const evolutionPokemon = pokemonList.find((p: PokemonListData) =>
            p.evolutions?.some(
                (e) =>
                    e.trigger === EvoTrigger.LEVEL &&
                    e.evolvedPokemon == pokemonName
            )
        );
        if (
            maxRegion != GameConstants.Region.none &&
            pokemonMap[evolutionPokemon.name].nativeRegion > maxRegion
        ) {
            return;
        }
        return (evolutionPokemon as PokemonListData)?.evolutions?.find(
            (e) => e.evolvedPokemon == pokemonName
        );
    }

    public static getPokemonStonePrevolution(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): EvoData {
        if (
    ...
    public static getPokemonBattleFrontier(
        pokemonName: PokemonNameType
    ): Array<number> {
        const cache = this.getCache<number[]>(
            this.getPokemonBattleFrontier.name
        );
        if (cache[pokemonName]) {
            return cache[pokemonName];
        }
        ...
        BattleFrontierMilestones.milestoneRewards
            .filter((m) => m instanceof BattleFrontierMilestonePokemon)
            .forEach((milestone) => {
    ...
    public static getPokemonWandering(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        if (
            maxRegion !== GameConstants.Region.none &&
            maxRegion < pokemonMap[pokemonName].nativeRegion
        ) {
            return [];
        }
    ...
    public static getPokemonDiscord(
        pokemonName: PokemonNameType
    ): Array<number> {
    ...
    public static getPokemonTempBattleReward(
        pokemonName: PokemonNameType
    ): Array<string> {
        const cache = this.getCache<string[]>(
            this.getPokemonTempBattleReward.name
        );
        if (cache[pokemonName]) {
            return cache[pokemonName];
        }
        ...
        Object.entries(TemporaryBattleList).forEach((tempBattle) => {
            const firstTimeRewardFunction =
                tempBattle[1].optionalArgs?.firstTimeRewardFunction?.toString();
            this.getPokemonRewards(firstTimeRewardFunction).forEach(
                (pokemon) => {
                    cacheLine[pokemon].push(tempBattle[0]);
                }
            );
            const rewardFunction =
                tempBattle[1].optionalArgs?.rewardFunction?.toString();
    ...
    public static getPokemonGymReward(
        pokemonName: PokemonNameType
    ): Array<string> {
    ...
    public static getPokemonDungeonReward(
        pokemonName: PokemonNameType
    ): Array<string> {
        const cache = this.getCache<string[]>(
            this.getPokemonDungeonReward.name
        );
        if (cache[pokemonName]) {
            return cache[pokemonName];
        }
    ...
    public static getPokemonQuestLineReward(
        pokemonName: PokemonNameType
    ): Array<string> {
        const cache = this.getCache<string[]>(
            this.getPokemonQuestLineReward.name
        );
        if (cache[pokemonName]) {
            return cache[pokemonName];
        }
        const cacheLine = this.initCacheLine(cache, Array<string>);
        App.game.quests.questLines().forEach((questLine) =>
    ...
    public static getPokemonTrades(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        const cache = this.getRegionalCache<string[]>(
            this.getPokemonTrades.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        Object.entries(TownList).forEach(([townName, town]) => {
            ...
            if (townShops.length) {
                ...
                for (let i = 0; i < townShops.length; i++) {
                    ...
                    if (shop instanceof GemMasterShop) {
                        ...
                        GenericDeal.list[shop.traderID]?.().forEach((deal) => {
                            deal.profits.forEach((profit) => {
                                if (
                                    profit.type === DealCostOrProfitType.Item &&
                                    this.pokemonNames.includes(profit.item.type)
                                ) {
                    ...
                    deals?.forEach((deal) => {
                        if (
                            this.pokemonNames.includes(deal.item.itemType.type)
                        ) {
    ...
    public static getPokemonGifts(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<object> {
        const cache = this.getRegionalCache<object[]>(
            this.getPokemonGifts.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
    ...
    public static getPokemonDreamOrbs(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Array<string> {
        const cache = this.getRegionalCache<string[]>(
            this.getPokemonDreamOrbs.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        if (
            maxRegion !== GameConstants.Region.none &&
            maxRegion < GameConstants.Region.unova
        ) {
            return cacheLine[pokemonName];
        }
        App.game.dreamOrbController.orbs.forEach((orb) =>
            orb.items.forEach((dreamOrbLoot) => {
                if (dreamOrbLoot.item.type === ItemType.item) {
                    const item = ItemList[dreamOrbLoot.item.id];
                    if (
                        item instanceof PokemonItem &&
                        this.pokemonNames.includes(item.type)
                    ) {
    ...
    public static getBattleCafeCombination(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): {
        spin?: GameConstants.AlcremieSpins;
        sweet?: GameConstants.AlcremieSweet;
    } {
        const cache = this.getRegionalCache<object>(
            this.getBattleCafeCombination.name
        );
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        ...
        if (
            maxRegion !== GameConstants.Region.none &&
            maxRegion < GameConstants.Region.galar
        ) {
            return cacheLine[pokemonName];
        }
        ...
        let sweet: GameConstants.AlcremieSpins,
            spin: GameConstants.AlcremieSweet;
        for (sweet of GameHelper.enumNumbers(GameConstants.AlcremieSweet)) {
            for (spin of GameHelper.enumNumbers(GameConstants.AlcremieSpins)) {
                const spinReward =
                    BattleCafeController.evolutions[sweet][spin]?.name;
    ...
    public static getPokemonSafariItem(
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.Region.none
    ): Record<GameConstants.Region, { chance: number; requirement?: string }> {
        const cache = this.getRegionalCache<
            Record<
                GameConstants.Region,
                { chance: number; requirement?: string }
            >
        >(this.getPokemonSafariItem.name);
        if (cache[maxRegion]) {
            return cache[maxRegion][pokemonName];
        }
        const cacheLine = this.initRegionalCacheLine(cache, maxRegion, Object);
        Object.entries(SafariItemController.list).forEach(([region, list]) => {
            ...
            list.forEach((item) => {
                ...
                if (this.pokemonNames.includes(pokemonItem)) {
                    cacheLine[pokemonItem][region] = {
                        chance:
                            item.weight /
                            list.reduce((acc, it) => acc + it.weight, 0),
                    };
                    if (item.requirement) {
                        cacheLine[pokemonItem][region].requirement =
                            item.requirement;
                    }
                }
            });
    ...
    public static getPokemonLocations = (
        pokemonName: PokemonNameType,
        maxRegion: GameConstants.Region = GameConstants.MAX_AVAILABLE_REGION
    ) => {
        const encounterTypes = {};
        // Routes
        const regionRoutes = PokemonLocations.getPokemonRegionRoutes(
            pokemonName,
            maxRegion
        );
        ...
        // Dungeons
        const dungeons = PokemonLocations.getPokemonDungeons(
            pokemonName,
            maxRegion
        );
        if (dungeons.length) {
        ...
        // Dungeon Boss
        const bossDungeons = PokemonLocations.getPokemonBossDungeons(
            pokemonName,
            maxRegion
        );
        ...
        // Dungeon Chest
        const chestDungeons = PokemonLocations.getPokemonChestDungeons(
            pokemonName,
            maxRegion
        );
        ...
        // Shadow Pokemon
        const shadowPokemon = PokemonLocations.getShadowPokemonDungeons(
            pokemonName,
            maxRegion
        );
        ...
        // Roaming
        const roaming = PokemonLocations.getPokemonRoamingRegions(
            pokemonName,
            maxRegion
        );
        if (roaming.length) {
        ...
        // Baby
        const parents = PokemonLocations.getPokemonParents(
            pokemonName,
            maxRegion
        );
        if (parents.length) {
        ...
        // Safari
        const safariChance =
            PokemonLocations.getPokemonSafariChance(pokemonName);
        if (Object.keys(safariChance).length) {
            encounterTypes[PokemonLocationType.Safari] = safariChance;
        }
        // Evolution
        const evolutions = PokemonLocations.getPokemonPrevolution(
            pokemonName,
            maxRegion
        );
        ...
        const battleFrontier =
            PokemonLocations.getPokemonBattleFrontier(pokemonName);
        if (battleFrontier.length) {
            encounterTypes[PokemonLocationType.BattleFrontier] = battleFrontier;
        }

        // Wandering
        const wandering = PokemonLocations.getPokemonWandering(
            pokemonName,
            maxRegion
        );
        if (wandering.length) {
        ...
        // Temp battle reward
        const tempBattle =
            PokemonLocations.getPokemonTempBattleReward(pokemonName);
        ...
        // Dungeon reward
        const dungeonReward =
            PokemonLocations.getPokemonDungeonReward(pokemonName);
        if (dungeonReward.length) {
            encounterTypes[PokemonLocationType.DungeonReward] = dungeonReward;
        }

        // Quest Line reward
        const questLineReward =
            PokemonLocations.getPokemonQuestLineReward(pokemonName);
        if (questLineReward.length) {
            encounterTypes[PokemonLocationType.QuestLineReward] =
                questLineReward;
        }

        // Trades
        const trades = PokemonLocations.getPokemonTrades(
            pokemonName,
            maxRegion
        );
        if (trades.length) {
        ...
        // Dream Orbs
        const dreamOrbs = PokemonLocations.getPokemonDreamOrbs(
            pokemonName,
            maxRegion
        );
        if (dreamOrbs.length) {
        ...
        // Battle Café
        const combination = PokemonLocations.getBattleCafeCombination(
            pokemonName,
            maxRegion
        );
        ...
        // Safari Items
        const safariItems = PokemonLocations.getPokemonSafariItem(
            pokemonName,
            maxRegion
        );
    ...
    public static isObtainableAndNotEvable = (pokemonName: PokemonNameType) => {
        ...
        const isEvable =
            locations[PokemonLocationType.Dungeon] ||
            locations[PokemonLocationType.DungeonBoss] ||
            locations[PokemonLocationType.DungeonChest] ||
            (locations[PokemonLocationType.Evolution] as EvoData[])?.some(
                (evo) => evo.trigger === EvoTrigger.STONE
            ) || // Only stone evolutions gives EVs
