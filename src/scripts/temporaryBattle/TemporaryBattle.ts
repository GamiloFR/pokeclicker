...
class TemporaryBattle extends TownContent implements TmpTemporaryBattleType {
    ...
    public cssClass(): string {
        return App.game.statistics.temporaryBattleDefeated[
            GameConstants.getTemporaryBattlesIndex(this.name)
        ]()
            ? "btn btn-success"
            : "btn btn-secondary";
    }
    ...
    public isVisible(): boolean {
        return (
            (this.isUnlocked() ||
                this.optionalArgs.visibleRequirement?.isCompleted()) &&
            !this.completeRequirements.every((r) => r.isCompleted())
        );
    }
    public onclick(): void {
    ...
    public getDisplayName() {
        return (
            this.optionalArgs.displayName ??
            this.name.replace(/( route)? \d+$/, "")
        );
    }
    ...
    public getImage() {
        ...
        const finalMonImageName =
            this.optionalArgs?.finalPokemonImage ?? imageName;
        return TemporaryBattleRunner.finalPokemon()
            ? `assets/images/npcs/${finalMonImageName}.png`
            : `assets/images/npcs/${imageName}.png`;
    }
    ...
    public getPokemonList() {
        return this.pokemons.filter((p) =>
            p.requirements.every((r) => r.isCompleted())
        );
    }
}
