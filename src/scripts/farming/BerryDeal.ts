class BerryDeal implements TmpBerryDealType {
    ...
    public calculateMaxTrades(): number {
        return Math.min(
            ...this.berries.map((b) =>
                Math.floor(App.game.farming.berryList[b.berryType]() / b.amount)
            )
        );
    }
    ...
    private static randomBattleItem(): Item {
        const battleItem = SeededRand.fromArray(
            GameHelper.enumStrings(GameConstants.BattleItemType)
        );
    ...
    private static randomEvoItem(): Item {
        const evoItem = SeededRand.fromArray(
            GameHelper.enumStrings(GameConstants.StoneType).filter(
                (name) =>
                    ![
                        'None',
                        'Black_DNA',
    ...
    private static randomUndergroundItem(): Item {
        return ItemList[
            SeededRand.fromArray(
                UndergroundItems.list.filter(
                    (item) =>
    ...
    private static randomPokeballDeal(): BerryDeal {
        ...
        return SeededRand.fromArray([
            new BerryDeal(
                [this.randomBerry(firstGen), this.randomBerry(secondGen)],
                [SeededRand.intBetween(20, 40), SeededRand.intBetween(5, 15)],
                ItemList.Fastball,
                1
            ),
            new BerryDeal(
    ...
    public static generateDeals(date: Date) {
        ...
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Goldenrod City']
        ].push(...this.generateGoldenrodDeals());
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Mauville City']
        ].push(...this.generateMauvilleDeals());
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Pinkan Pokémon Reserve']
        ].push(...this.generatePinkanDeals());
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Hearthome City']
        ].push(...this.generateHearthomeDeals());
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Secret Berry Shop']
        ].push(...this.generateSecretBerryShopDeals());
        BerryDeal.list[
            GameConstants.BerryTraderLocations['Driftveil City']
        ].push(...this.generateDriftveilDeals());
    }
    ...
    private static generateGoldenrodDeals() {
        ...
        list.push(
            new BerryDeal(
                [this.randomBerry(firstGen), this.randomBerry(secondGen)],
                [SeededRand.intBetween(30, 70), SeededRand.intBetween(10, 30)],
    ...
    private static generateMauvilleDeals() {
        ...
        while (i < maxTries && temp.length < 3) {
            const deal = new BerryDeal(
                [this.randomBerry(thirdGen), this.randomBerry(fourthGen)],
                [SeededRand.intBetween(30, 70), SeededRand.intBetween(10, 30)],
            ...
            if (
                temp.every(
                    (madeDeal) => madeDeal.item.name !== deal.item.itemType.name
                )
            ) {
                temp.push(deal);
            }
    ...
    private static generateHearthomeDeals() {
        ...
        const fourthGen = Farming.getGeneration(3);
        const fifthGen = [
            // only use berries that can grow in under a day so players have time to grow them
            BerryType.Micle,
            BerryType.Custap,
            BerryType.Jaboca,
            BerryType.Rowap,
    ...
    public static canUse(
        town: GameConstants.BerryTraderLocations,
        i: number
    ): boolean {
        ...
        if (!deal) {
            ...
            return deal.berries.every(
                (value) =>
                    App.game.farming.berryList[value.berryType]() >=
                    value.amount
            );
        }
    }
    ...
    public static use(
        ...
        if (BerryDeal.canUse(town, i)) {
            ...
            const maxTrades = trades.reduce(
                (a, b) => Math.min(a, b),
                tradeTimes
            );
            deal.berries.forEach((value) =>
                GameHelper.incrementObservable(
                    App.game.farming.berryList[value.berryType],
                    -value.amount * maxTrades
                )
            );
            if (deal.item.itemType instanceof UndergroundItem) {
                UndergroundController.gainMineItem(
                    deal.item.itemType.id,
                    deal.item.amount * maxTrades
                );
            } else {
                deal.item.itemType.gain(deal.item.amount * maxTrades);
            }
            GameHelper.incrementObservable(
                App.game.statistics.berryDailyDealTrades
            );
            ...
            Notifier.notify({
                ...
                setting:
                    NotificationConstants.NotificationSetting.Items.item_bought,
            });
        }
    }
}

BerryDeal satisfies TmpBerryDealStaticType;
